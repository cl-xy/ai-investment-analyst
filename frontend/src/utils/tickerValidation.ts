...
export function getTickerError(input: string): string | null {
  ...
  if (/[^\x00-\x7F]/.test(trimmed)) return 'Only ASCII letters, numbers, dots, and hyphens allowed'
  ...
  if (!/^[A-Za-z0-9.\-]+$/.test(trimmed)) return 'Only letters, numbers, dots, and hyphens allowed'
  return null
}
